...
export const getPriceForSymbol = async (fsym, tsym) => {
  return new Promise((resolve, reject) => {
  	let prices = {};
	  redisClient.get('price_data', async (err, reply) => {
		  if (err || !reply) {
			  const symbols = await coinmarketcap.ticker({
				  convert: tsym,
			  });

			  await symbols.forEach((item) => {
				  prices[item.symbol] = item;
			  });

			  redisClient.set('price_data', JSON.stringify(prices), 'EX', 30);
			  console.log('from cmc ', Object.keys(prices).length);
		  } else {
			  prices = JSON.parse(reply);
			  console.log('from redis', Object.keys(prices).length);
		  }

		  const priceData = prices[fsym];
		  ...
		  return resolve({
			  ...
			  change: priceData['percent_change_24h'],
			  period: '24h',
			  marketCap: priceData['market_cap_usd'],
			  volume24Hr: priceData['24h_volume_usd'],
		  });
	  });
  });
};

export const getPriceForSymbolViaCC = (fsym, tsym) => {
...
export const getAllTokenBalances =
  async(address, includeZeroBalances = false) => {
    const balances = [];
  ...
  // ethereum address and keep track of which contract addresses (tokens) were involved
    const queries = Object.keys(TOKEN_CONTRACTS).map((symbol) => {
      return async() => {
        let balance = Number(
          await getTokenBalance(getContractAddress(symbol), address)
        );
        if (Number.isNaN(balance) || !(balance || includeZeroBalances)) {
          return;
        }
        try {
          const {price, change, period} = await getPriceForSymbol(symbol, 'USD');
          balances.push({
            symbol,
            balance,
            price,
