...
export const getTopNTokens = async (n) => {
  ...
  const tokens = Object.keys(prices)
    ...
    .sort((a,b)=>Number(prices[a].market_cap_usd) < Number(prices[b].market_cap_usd) ? 1 : -1)
    .slice(0, n)
    .map((symbol)=>({
      symbol,
      marketCap: prices[symbol].market_cap_usd,
      price: prices[symbol].price_usd,
