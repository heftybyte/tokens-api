...
export const getTokensData = async () => {
  let err = null;
  let prices = await getAllPrices('USD', COIN_MARKETCAP).catch(e=>err = e)
  let tokens = Object.keys(prices).filter((symbol)=>{
...
export const getTopNTokens = async (n) => {
  let {err, tokens} = await getTokensData();
...
export const getWatchListTokens = async (watchList) => {
	let {err, tokens} = await getTokensData();
	...
	let filteredTokens = watchList.map((symbol) =>{
