...
export const getPrice = async (fsym, tsym, source) => {
  ...
  for (; sourceIndex !== -1 && sourceIndex < SOURCES.length; sourceIndex++) {
    ...
    console.log(new Date(), `price not found for ${fsym} to ${tsym} at ${SOURCES[sourceIndex]}, next source: ${SOURCES[sourceIndex+1]}`,sourceIndex)
  }
...
const getPricesFromCryptoCompare = async (tsym) => {
  ...
  symbols.forEach(symbol=>{
    if (!priceData[symbol] || !priceData[symbol][tsym]) {
      console.log(new Date(), 'no price data for', symbol, priceData[symbol])
      return
    }
